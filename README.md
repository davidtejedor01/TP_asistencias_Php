...
Esta carpeta contiene los _scripts_ de servidor que manejan la lógica principal de la aplicación.
...
- `funciones.php`: **Librería de Funciones.**
  ...
  - `leerAlumnos()`: Ejecuta una consulta SQL para recuperar los datos de todos los alumnos, ordenados alfabéticamente por apellido y nombre.
  - `guardarAsistencias($fecha, $presentes)`: Lee todos los alumnos para determinar quiénes estan presentes y ausentes según los checkboxes enviados. Finalmente inserta el registro en la tabla (`asistencias`).

- `ProcesarDatos.php`: **Procesamiento de Asistencia.**
  - **Función:** Es el encargado de recibir los datos del formulario (`POST`).
  - **Lógica:** Recibe la fecha y el listado de alumnos presentes. Llama a la funcion `guardarAsistencias($fecha, $presentes)` para realizar el procesamiento de datos y, finalmente, redirecciona al usuario al formulario principal.

### `📁 sql`

Contiene los _scripts_ SQL para la configuración inicial y relleno de la base de datos.
...
- `Asistencia.php`: **Formulario Principal de Asistencia (Interfaz de Usuario).**
  ...
  - **Lógica PHP:** Incluye `php/funciones.php` y utiliza la función `leerAlumnos()` para precargar y generar dinámicamente el listado de estudiantes con sus respectivos _checkboxes_.
...
1.  Clona el repositorio: `git clone https://www.youtube.com`
